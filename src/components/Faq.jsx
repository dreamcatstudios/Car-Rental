...
const Faq = () => {
  ...
  const accordion = {
    normal:
      "bg-white p-3 pt-0 pb-0 h-0 overflow-hidden transition-all ease-in-out delay-45 ",
    hover:
        ...
        {accordionContent.map((item, index) => {
          return (
            <div className="shadow-lg ">
              ...
              >
                <h1 onClick={handleClick}>{item.question}</h1>
              </div>
              <div className={click ? accordion.hover : accordion.normal}>
                <p>{item.answer}</p>
              </div>
            </div>
          );
