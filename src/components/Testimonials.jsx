import React from "react";

const Testimonials = () => {
  return (
    <section className="container bg-[#f8f8f8]">
      <div className="pt-32 pb-32 flex flex-col gap-2">
        <h1 className="font-bold text-xl text-center">Reviewed by People</h1>
        <h1 className="text-4xl text-center">Client's Testimonials</h1>
        <p className="text-center">
          Discover the positive impact we've made on the our clients by reading
          through their testimonials. Our clients have experienced our service
          and results, and they're eager to share their positive experiences
          with you.
        </p>
        <div className="flex-col gap-4 mt-12 ">
          <div className="p-12 bg-white shadow-md">
            <h1 className="font-bold text-xl">
              "We rented a car from this website and had an amazing experience!
              The booking was easy and the rental rates were very affordable. "
            </h1>
            <div className="flex">
              <div>
                <img src="" alt="" />
              </div>
              <div>
                <h1 className="font-bold text-xl">Parry Hotter</h1>
                <p>Belgrade</p>
              </div>
            </div>
          </div>

          <div className="p-12 bg-white shadow-md">
            <h1 className="font-bold text-xl">
              "We rented a car from this website and had an amazing experience!
              The booking was easy and the rental rates were very affordable. "
            </h1>
            <div className="flex">
              <div>
                <img src="" alt="" />
              </div>
              <div>
                <h1 className="font-bold text-xl">Parry Hotter</h1>
                <p>Belgrade</p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Testimonials;
